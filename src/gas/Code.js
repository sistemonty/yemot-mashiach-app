  ...
  if (e?.parameter?.ApiPhone) {
    try {
      ...
      for (var i = 0; i < numbers.length; i++) {
        ...
        if (cleanNum === phone) {
          // מספר מורשה - מעביר לשלוחה הרצויה
          return ContentService.createTextOutput('go_to_folder=/1/2')
            .setMimeType(ContentService.MimeType.TEXT);
        }
      }
      
      // מספר לא מורשה - משמיע הודעה ומנתק
      return ContentService.createTextOutput('read=אינך מורשה להשתמש במערכת&hang_up')
        .setMimeType(ContentService.MimeType.TEXT);
      
    } catch (error) {
      // במקרה של שגיאה - מנתק את השיחה
      return ContentService.createTextOutput('read=אירעה שגיאה במערכת&hang_up')
        .setMimeType(ContentService.MimeType.TEXT);
    }
  }
